...
describe("castWardrobeAfter", () => {
  it("re-dresses a character from a change declared in an earlier draft", () => {
    const { cast } = castWardrobeAfter([TRACEY, JAIME], [{}, change("Tracey", "nude")]);
    expect(cast.find((c) => c.name === "Tracey")!.wardrobe).toBe("nude");
  ...
  it("leaves the cast untouched when no change has been declared", () => {
    const { cast, others } = castWardrobeAfter([TRACEY, JAIME], [{}, {}, {}]);
    expect(cast.map((c) => c.wardrobe)).toEqual([TRACEY.wardrobe, JAIME.wardrobe]);
    expect(others).toEqual({});
  });
});
...
describe("wardrobe directives", () => {
  it("routes a costume change to wardrobeChanges and nowhere else", () => {
    const planning = castSystemDirective([TRACEY]);
