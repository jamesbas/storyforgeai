...
| --- | --- | --- |
| **1.81** | 2026-08-16 | **Keyframe prompts stop describing time passing, and the direction you pick now reaches the story.** A still image has no camera move and no duration in it, but prompts were being written with both — the template literally appended "slow push-in begins from here" to every opening frame it produced, and the check meant to catch that language only ever ran on sexually explicit scenes. It now runs on every project, and rejects a named camera move, a stated duration or a transition while still allowing a pose caught mid-action, which is a legitimate thing for a frame to show. The instruction to the prompt writer no longer calls the two frames "the same moment seconds apart", which is a contradiction; they are described as two frozen instants, with actions to be written as poses, contact and visible consequences. Three planning faults are fixed alongside: choosing a creative direction and then running the Director produced a story arc written as though nothing had been chosen, and that arc was reused from then on; the tradeoffs a direction accepts were being passed downstream as things to avoid, working against the direction you picked; and a story arc returning fewer emotional beats than segments left every later batch of scenes with no emotional direction at all. Per-scene director and camera plans that skip segments are now reported instead of stored silently. |
