...
| **2.34** | 2026-08-29 | **LM Studio planning can now follow two app-wide custom system prompts.** Settings keeps one policy for Agentic Canvas and one for Storyboard planning and render-prompt creation; both are required together, and clearing both restores StoryForgeAI's built-in prompts. Custom direction is added to future model calls without removing the artifact, continuity, or structured-output rules that keep generated work valid. |
