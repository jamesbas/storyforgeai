...
| **2.55** | 2026-09-21 | **A scene can be removed from a storyboard, completing the set of things you can do to a running order.** **Delete** sits at the right-hand end of the controls on each card, set apart because it is the only one that cannot be undone. The scenes after it move up, the piece gets one segment shorter, and every other scene keeps everything it has already rendered. The scene's card, prompts, seed, LoRA choices and any audio cues anchored to it go with it — a cue left behind would have broken the moment anyone opened it, because it reads its length from the scene it names. **Its rendered frames and clips are not erased:** they stay in the project folder exactly as deleting a whole project leaves its media, on the grounds that renders are expensive to reproduce, and the confirmation says so rather than letting you assume the opposite. The last remaining scene cannot be deleted, because a storyboard with no scenes is not a storyboard. This release also fixes a fault the first two could not reach: the Director's and Cinematographer's per-scene notes are keyed by scene number, and a departed scene's note either collided with the renumbering or lingered describing a scene that no longer existed. |
