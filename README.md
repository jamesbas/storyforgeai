...
| **2.49** | 2026-09-18 | **Media generated by a synchronous Wan2GP MCP v2 server now returns to the storyboard instead of leaving one orphaned image in Wan2GP's output folder.** The MCP SDK ended tool requests after one minute while Wan2GP correctly kept rendering, so StoryForgeAI lost the first frame's result, never requested the second frame, and had nothing to attach to the scene. StoryForgeAI now asks Wan2GP to return a resumable job after 30 seconds, safely inside the SDK deadline, then uses the normal job-status polling path until each frame completes. Both first-scene keyframes are consequently generated and persisted without requiring `--mcp-async`. |
