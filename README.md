...
![StoryForgeAI business infographic showing the path from business intent through the AI creative crew, storyboard, Wan2GP generation, and human-approved media package](docs/presentations/generated/StoryForgeAI-Business-Infographic.png)

*The business view: creative planning, generation, and approval stay connected in
one production record. An [editable PowerPoint version](docs/presentations/generated/StoryForgeAI-Business-Infographic.pptx)
is included for stakeholder presentations.*
...
| --- | --- | --- |
| **2.62** | 2026-09-23 | **Reference mode now holds a character's face for the whole clip even when the keyframes are set to description and face swap.** One setting governed two different things: whether a character's photograph conditions the two keyframes, and whether it is sent to a reference-to-video model. Choosing **Description and face swap only** is the right call for the keyframes — a photograph conditions the whole image, so on a shot with several people the likeness can land on more than one of them — but it also, silently, stopped the photograph reaching the video model. A project pinned to reference mode with a character selected was therefore paying that mode's extra minutes per clip while receiving nothing for them: the prompt fell back to "the start frame fixes how they look", which is exactly the dependency reference mode exists to escape, and identity was free to drift across the twenty seconds in between. The two are now separate, because the risk that justifies declining a keyframe photograph does not apply to the video model — it is handed each photograph as a *named* subject tied to that character in the prompt, rather than a bare list it can apply to anyone in frame. A new choice under **Whether the video model is shown each character** appears when reference mode is selected, and is on unless you turn it off, so an existing project gets the character lock its model choice already implied. |
| **2.61** | 2026-09-22 | **The Director's notes for the opening scenes are no longer written one beat late.** The previous release split a long project's direction into windows and handed each window the exact beats it covers, numbered — but only the windows after the first. The opening call was capped at eight scenes and left to count down an unnumbered list, and it miscounted: on a 27-scene project the first eight intents each described the *following* scene's beat, while scenes 9 to 27 were exact. The effect was subtle enough to miss — every note was a real note about a real beat, just attached to the scene before it — and it pushed the opening of the film out of step with its own arc. The opening window is now given its beats by number exactly as every other window is, which also covers short projects that are never split at all. |
| **2.60** | 2026-09-22 | **A keyframe whose whole point is one place two bodies touch no longer puts the camera where it cannot see it.** Every scene has always specified a shot size, a lens, a camera height and a move — and all of those describe how high the camera is or how much of the frame it fills. Nothing said where it stood *around* the subject, which is the thing that decides whether a given side of a body faces the lens. It rarely matters and it decides the shot completely when the frame is about one point of contact: a scene whose entire content was a man's hand gripping a woman's backside named the anatomy, the grip and the pose, opened "Medium close-up, eye level" with the two of them staged facing each other, and rendered his hand on her **hip** — the only place the camera could see a hand. Nothing was wrong with the prompt except that the lens was on the wrong side of her. Another scene in the same project opened "Medium close-up, low angle" on a frame about exposed skin; low angle is height again. The Cinematographer now names the angle as well as the height, and says so per scene where a beat turns on one specific point of contact. The image prompt agent is told to decide what the frame is actually about before writing it, put the camera where that thing is visible, and turn the bodies to match — naming which side of the frame the focal point occupies. A frame that still describes a contact its camera cannot see is now rejected before it reaches the GPU and rewritten, rather than rendering something plausible and wrong. |
| **2.59** | 2026-09-22 | **A scene card no longer races ahead and performs three later scenes at once.** Scene cards are written four at a time, and each batch was correctly given only its own four story beats — but the Director's and Cinematographer's plans went over whole, and those are keyed by scene across the entire project. So the call writing scenes 5 to 8 received four beats and **all twenty-seven scene intents**: the rest of the film, in detail. On a 27-scene project that produced a scene 8 titled "The Encirclement" which opened on the men closing in, then spun her, then had a third act of action entirely — the content of scenes 11, 12 and 13 compressed into one twenty-second shot, while its own beat was only "Leroy lifts the hem of her sundress while Ray and Bill stand up". The scenes that should have carried that action were then left repeating what had already happened, and the start and end frame prompts built from the card described a moment the story had not reached. Each batch now sees only the intents and shot plans for its own scenes, while the project-wide direction — creative thesis, pacing, camera language, lighting rules — is still passed in full. The beats are also handed over keyed by scene number rather than as a bare list, so a card cannot be written against its neighbour's beat by miscounting, and the instruction now says outright that a card covers its own beat and stops there. |
| **2.58** | 2026-09-22 | **The Director now follows the story arc all the way through a long piece, instead of losing it half way and starting again.** On a 27-scene project the Director was asked for all 27 scene intents in one call and returned sixteen — not the first sixteen scenes, but the whole film compressed into sixteen, ending on the closing shot of an empty room at intent 16. The repair call then asked for scenes 17 to 27 while telling it only that those numbers had no entry, so it began the story again from the middle: scene 17's beat was the climax and its direction was a reaction shot from ten beats earlier. The Director and Cinematographer are now written a window at a time like the arc, with the first call capped and told the rest is coming, and **each window is given the exact beats it is directing**, by number. Handing over the beats introduced a second problem that this release also fixes: given the beat, the model's easiest move is to hand it straight back, and every intent in the first and last windows did — a scene intent that restates its beat is worse than a missing one, because the prompts built from it spend their attention on words the storyboard already had. Both agents are now told plainly what an entry has to add, and any that still come back as their own beat are named on the canvas. Measured on the same project and model, the intents went from four-fifths recycled beat to one-fifth. |
...
- **WanGP MCP integration** — automatic v1/v2 contract detection, model discovery,
  schema/default settings retrieval, settings-manifest generation, and synchronous
  or submit/poll/cancel generation.
...
- **Scene continuity you can override** — a scene can start from the previous scene's
  ...
  wardrobe, location and lighting carry across the seam. The reference outranks the
  prompt, though, so **Match the carried-over frame** should be cleared on any scene
  whose end frame has to show something the reference does not already contain — a
  different part of the set, or a person who is not in it. Asked to walk two
  characters from a doorway to a bed, a conditioned end frame returned the doorway;
  the same prompt and seed with the reference withheld obeyed it.
...
- **Adult-content directives** — when the audience or tone calls for it, the
...
- **Stakeholder presentation package** — an editable overview deck plus dedicated
  [business and technical infographics](docs/presentations/README.md), with PNG
  exports and a reusable PowerPoint generator.
...
| `npm run storyboard:health` | Report where a project's prompts came from — model, hybrid or fallback — and which scenes carry gate codes |
...
Scripts that read configuration run under `tsx --env-file-if-exists=.env.local`,
because **`tsx` does not load `.env.local` — only Next does.** Without it a
diagnostic reports the built-in defaults while appearing to report yours, which is
worse than reporting nothing. If you add a script that touches `lib/config.ts`,
add the flag.
...
**A negative prompt has no addressee.** "Dark skin for <character name>" aims
nothing at that character; the sampler sees "dark skin" and steers the whole frame
away from it,
...
### App-wide system prompts

**Settings → LM Studio system prompts** can add standing instructions to future AI
planning calls, in three separate scopes: **Agentic Canvas** (Variant Explorer,
World Builder, Director, Cinematographer, Art Director), **Storyboard** (Intake,
Story Architect, Visual Bible, Storyboard Artist) and **Render prompts** (the
Image and Video Prompt agents). The scopes exist because those workflows want
contradictory instructions: the Storyboard agents are told *not* to write render
prompts, so a scene-graph rule aimed at keyframes makes them answer the wrong
question. All three are required together, and clearing all three returns every
agent to the built-in prompts. Changes apply to future model calls in every
project and never rewrite artifacts that already exist.

StoryForgeAI already sends a `system` message on each OpenAI-compatible inference
request. Its built-in text is also the artifact contract: it names required JSON
fields, continuity rules, and the output shape validated after generation. A
custom prompt is therefore added as the leading creative instruction rather than
replacing that contract. Leaving the settings empty preserves StoryForgeAI's
current request exactly; it does not remove the app's system message in favor of
a separate default configured in LM Studio — a system prompt saved in LM Studio's
own Config Presets governs its chat window and never reaches these calls.

Instructions written for the wrong scope degrade output without raising an error,
so Help § 10 covers the four common mistakes — render rules in a planning scope,
restating a character's appearance that the character library already appends,
redefining a schema field such as `charactersPresent`, and writing so much that it
crowds the model's context on all ~50 calls of a 24-scene run.
...
## Regenerating only the keyframes

The mirror image. Changing an image prompt, a scene's seed or an image LoRA
moves the frames and nothing else — and the clip is the longest job in the
scene to spend rendering from frames nobody has looked at yet.

**Regenerate all keyframes** on the Storyboard screen re-renders every scene's
frames and stops there. A collapsed *Regenerate keyframes for selected scenes*
panel underneath does the same for a subset, which is the usual case: three
shots came back wrong and the other seventeen are fine.

The new attempts deliberately carry no clip. Copying the old one forward would
claim a video built from frames that no longer exist; leaving it empty is both
true and useful, because **Generate all media** already recognises that state —
frames banked, clip outstanding — and finishes those scenes for the price of the
clip alone. So the working order is: fix the frames, look at them, then let
*Generate all media* fill in the clips.

Under **continue from previous end frame**, a scene's start frame is a copy of
the one before it's end frame, taken when it was rendered. Re-rendering scene 3
therefore leaves scene 4 showing a picture of a frame that exists nowhere — and
scene 4 still looks finished, so the screen names it. It is not swept into the
selection the way a broken clip chain is: a clip chain breaks outright when a
link goes missing, whereas an inherited frame merely goes stale, and re-rendering
seventeen scenes to fix three is not a trade the app should make on your behalf.
...
**A carried frame has to show everyone's head.** It is the only picture the next
scene has of these people, and an edit model carries forward a person it can see
while quietly deleting one it cannot — a man cropped at the neck in a carried
frame was dropped from the scene that inherited him, and his stated wardrobe
arrived on somebody else. The face swap cannot rescue that, since it can only
correct a face that is in shot. So an end frame the next scene will inherit is
asked for a frame wide enough to hold everyone, with every head inside it and
faces unobstructed, background players included. Heads rather than whole bodies:
requiring head-to-feet would forbid every close-up the storyboard is entitled to
ask for. The clause follows the opening shot description rather than trailing the
prompt, because appended after the cast sheet it was ignored. A scene with **Face
in frame** cleared is exempt, being framed without a face on purpose.
...
value and nothing more: the project's Settings screen lists every usable model
...
selection. MCP v1 identifies missing weights; MCP v2 does not expose installation
state, so its full catalog remains visible and a selection may download before it
renders.
...
StoryForgeAI discovers LoRAs by reading WanGP's own folder, including the sidecar
metadata and trigger words that the MCP catalog does not provide. Point at it to
enable the feature:
...
To do several at once without doing all of them, **Rewrite prompts for selected
scenes** above the scene list takes a tick-box selection with **Select all** and
**Clear all**, the same shape as the clip queue beside it. Fixing five scenes by
reaching for the all-scenes button is how the hand edits on the other thirteen get
thrown away.
...
An absence of clothing is held as a wardrobe state rather than as an outfit, because
phrasing it as `Wearing exactly: …` hands the model a garment to act on when there
is none. The Storyboard Artist records a change to no outfit as a wardrobe change in
its own right; for storyboards written before that, the Storyboard screen lists any
scene whose action requires it while the wardrobe still says otherwise.
...
| WanGP MCP | Start the WanGP MCP server, then set `WANGP_MCP_ENABLED=true` + `WANGP_MCP_URL`. MCP v1 and v2 are detected automatically; default v2 servers return a resumable job after 30 seconds, while `--mcp-async` returns it immediately |
