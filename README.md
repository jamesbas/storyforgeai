...
| --- | --- | --- |
| **2.46** | 2026-09-07 | **A keyframe preview you like can now be kept as the scene's actual frame, in one click.** Previewing a single frame is how a prompt edit gets judged without paying for a whole scene, but until now a preview that came back right was a dead end: it was a scratch pad, never part of an attempt, so the only route to making it the real frame was downloading it and importing it straight back — the same bytes, out through the browser and in through a file picker, to reach a state the app could have reached on its own. **Keep start frame** and **Keep end frame** now appear beside the preview controls once a frame has been previewed, and put that image onto the scene's latest attempt in place of the rendered one. Everything downstream follows as it does for an import: under *Reuse end frame* a kept end frame is carried into the next scene's start frame, and you are told which scene that was; if the scene already has a clip, the card says the video was built from the frame you have just replaced. A kept preview is tracked separately from an imported one, because the two differ where it matters — a preview was rendered here from the scene's pinned seed against its current prompt, so the seed still describes it, and the card no longer claims otherwise. Once kept, the image stops being listed as a preview, so the same picture is not shown twice. |
