...
| --- | --- | --- |
| **2.45** | 2026-09-06 | **The narrative arc is now visible on the Agentic Canvas, and can be rewritten — previously it was the one artifact you could never ask to be redone.** The arc is what every other agent builds on, but it was produced invisibly the first time the Director ran and then defended: once a model had written one, nothing would replace it. That guard exists for a good reason — without it every crew run would churn a perfectly good arc — but it also meant no improvement to how the story is planned could ever reach a project that already had one, and the only way round it was editing `project.json` by hand. There is now a **Story Architect** card, sitting between Variant Explorer and World Builder where it belongs in the order things are built. It shows the logline and every beat, marks the beats that carry the previous one on so sustained action is visible at a glance, and its **Regenerate** button writes a fresh arc outright. It is deliberately not part of **Run core agents**: a new arc is a different story, so the plans and scene cards already written describe the one it replaced. The card says so, naming exactly which of them now predate the arc and need regenerating in order. Projects with no arc still get one automatically inside the Director's run, exactly as before — what is now a decision is *replacing* a good one. |
| **2.44** | 2026-09-05 | **Reference-mode clips now always receive the pinned characters' photographs, however the checkpoint was chosen.** Whether those photographs were sent was decided by the project's **tier** setting, but how the job is actually built is decided by the family of the video model it resolves to — two separate fields that drift apart easily. Choosing a Ref2VA checkpoint from the video model dropdown saves the model alone and never touches the tier, and a Ref2VA model arriving as an app-wide default or an environment pin sets no tier at all. Any of those routes produced a reference-mode render with the two keyframes but no character references whatsoever: it completed, cost the usual three-times-FL2VA render time, and came back with identity held no better than a keyframe job — indistinguishable from the model simply failing at the thing reference mode exists to do. The cast is now resolved for every clip and applied by the code that knows which model the job landed on, so the two signals cannot disagree. The **How MiniMax H3 is given the shot** radio on Project Settings also now reflects the pinned model rather than the stored tier, so it can no longer show *First and last frame* selected while a reference checkpoint is what will actually run. Nothing changes for FL2VA projects, which are still deliberately sent no references and inherit identity through the two keyframes. |
| **2.43** | 2026-09-05 | **An opening frame of the wrong shape is now centre-cropped to fit instead of being refused, and it is shown to you straight away so you can check the crop before generating anything.** Refusing a mismatch was the strict reading of "used exactly as supplied", and in practice it refused almost everything worth supplying — cameras shoot 4:3 and 3:2, and image models emit squares and 832×1216, so a 16:9 project rejected nearly every real photograph. Any image is now accepted and the largest correctly-shaped region about the centre is kept; nothing is scaled, so no detail is lost beyond the edges that had to go. A frame that is already the right shape is still stored byte for byte, with no decode and no re-encode, so "exactly as supplied" remains literally true wherever it can be. The pinned image now appears in the panel on scene 1 the moment it is accepted, with a note saying what it was cropped from and to — previously it was invisible until a render had produced an attempt, so the first sight of the crop came after the keyframes built from it. If the crop lost something important, release it and pin a version you have cropped yourself. Files carrying an EXIF rotation now have that rotation baked in rather than left as a flag that downstream models may or may not honour. |
| **2.42** | 2026-09-05 | **A rejected opening frame now says so where you pinned it, instead of appearing to do nothing.** Picking an image of the wrong shape was refused correctly, but the message was written to the page-level error line at the very top of the storyboard — which on a twenty-scene project is far above the control you just used. The file picker reset itself, nothing else changed, and the feature looked broken rather than picky. The panel on scene 1 now reports the refusal itself, quoting the shape the project needs and the size of the file you chose, shows "Uploading…" while the transfer is in flight, and locks the picker until it finishes. It also states the accepted formats, the 16 MB limit and the required aspect ratio up front, so a mismatch can be seen before choosing rather than after. A refused pin is also now recorded in the server log with the reason — previously a rejection produced no log line at all, so nothing on the console distinguished "refused" from "never happened". |
| **2.41** | 2026-09-05 | **An action that needs longer than one clip can now be given more than one, and the end of a story stops eating the time the middle needed.** Every scene is exactly the same length and the scene count is just the runtime divided by it, so nothing in the pipeline could ever spend more time on one moment than another — a beat that should breathe and a throwaway reaction shot were both allotted a single clip. The Story Architect can now hand one continuous action several consecutive scenes, writing each as a stage of the same unbroken moment, and the Storyboard Agent is told which cards those are so it carries the action forward instead of re-establishing the setting and starting again. It is also now given a hard limit on how many closing scenes may be aftermath — reaction, tidying up, departure, an empty room — and told that if it runs out of story before it runs out of scenes, the earlier action was written too fast and should be given the spare ones instead. Separately, the New Project form now checks the brief against the runtime before anything is generated: if your concept describes more distinct actions than the duration can hold, it says how many of each and offers a one-click **Raise to Ns**. It is an estimate and deliberately a cautious one — it counts what you actually separated into sentences and sequencing words rather than guessing at implied action — and it never blocks creation. |
