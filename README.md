...
| **2.50** | 2026-09-18 | **The Wan2GP timeout handoff now reaches the MCP SDK correctly, so scene generation proceeds from the first keyframe to the second.** Version 2.49 supplied the 45-second request deadline in the SDK's second `callTool` argument, but the installed SDK reserves that position for a result schema and accepts request options only as its third argument. Wan2GP still generated frame one, then StoryForgeAI rejected the returned timeout snapshot and stopped before frame two exactly as before. The transport now uses the installed three-argument signature, allowing the 30-second Wan2GP snapshot to become a tracked job, finish through polling, and return its image path before the second keyframe is submitted. |
