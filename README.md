...
| **2.47** | 2026-09-14 | **StoryForgeAI now works with Wan2GP 13.01's default MCP v2 contract without requiring compatibility flags.** Model discovery follows cursor pages, generation adapts to synchronous or async mode, and model settings continue to load from the consolidated toolboxes. MCP v1 remains supported. Because v2 no longer reports installed checkpoints, Settings keeps unknown models visible and explains that WanGP may download weights before rendering instead of showing empty model lists. |
