...
| --- | --- | --- |
| **2.25** | 2026-08-24 | **Characters who are not in your library are no longer stripped naked in every frame.** Only pinned cast members carry a wardrobe field, so anyone the story invented rather than the library — a husband, a waiter, a bystander — had no clothing the app recognised as authorised. The repair that removes an outfit a scene contradicts therefore treated *every* garment written for them as invented and rewrote it to "naked". On a twenty-four scene project that undressed a fully clothed husband in nine consecutive scenes and stripped a waiter's uniform the moment he walked through the door, in scenes where nothing was happening yet — and because the prompts still named him only by a first name that reads either way, some frames came back with the husband rendered as a woman. What the visual bible says people wear now counts as established, so described clothing survives and only garments the model made up are removed. Clothes still come off the way they always did, by a wardrobe change on the scene where it happens. Also fixed: the sentence stating that everyone in an act is naked could be added to a prompt that already carried it, and a repeated sentence is weighted twice by the renderer. |
| **2.24** | 2026-08-24 | **A costume change no longer haunts whichever scene later takes its number, and an act a character only talks about is no longer staged.** Two faults that met in one prompt. When the Storyboard Artist writes an undress into a scene, that change is folded into the project — but scene numbers are reused when you regenerate, so a change folded from an older draft re-attached to whatever scene now sat in that slot. Live, a scene rewritten into a fully clothed conversation kept an undress from the version before it, and every keyframe prompt written for it afterwards was "corrected" against a wardrobe the card no longer described: the model dressed her, a rule stripped her, and the storyboard and the prompts disagreed with no way to see why. What the storyboard folds in is now marked as its own and rebuilt from the current scenes; what you set by hand is still never touched. Separately, the check for whether a scene shows a sex act read the whole card, including dialogue — so a close-up of two faces, where the only explicit words were what someone *said*, was treated as an explicit scene and asked to show anatomy, contact and a position. Speech and daydreams are now excluded, while an act described in the same sentence as the speech is still seen. |
| **2.23** | 2026-08-24 | **The Help page now explains the two failures that look most like success.** A run can report itself as model-written and still be built on nothing: the narrative arc is written once before the scene cards, and when that step falls back the Storyboard Artist invents an arc as it goes and repeats itself near the end of a long project — so the tell is several late scenes with near-identical titles, not an error. Help now describes that, and what to do about it. It also documents the fallback banner's prompt count, which exists because a fallback storyboard does not always mean fallback prompts; the new *Rewrite all image prompts* button and why it deliberately leaves clip prompts alone; and two Wan2GP faults that read as something they are not — every scene failing with "direct filesystem paths are disabled" is a missing launch flag, and an empty model picker is a question about the connection rather than an answer about your installed models. |
| **2.22** | 2026-08-23 | **A story arc the planning model failed to write is no longer permanent, and no longer invisible.** The narrative arc is written once and every later run reuses it — correct when a model wrote it, and quietly ruinous when the Story Architect failed and the built-in builder supplied placeholders instead: *"Advance beat 7 of the narrative and raise the stakes"*, once per segment. The Storyboard Artist was then handed no story at all. It invented one as it went, four cards at a time, and on a twenty-four scene project ran dry near the end and repeated itself — five scenes with the same title. Neither re-running the canvas nor regenerating the storyboard could dislodge it, because both reused the stored arc. Worse, nothing said so: the canvas path asked the Story Architect for an arc without recording whether it answered. It now records that, and an arc the builder wrote is replaced the next time a planning model is available rather than being carried for the life of the project. |
| **2.21** | 2026-08-23 | **When the Director or Cinematographer stops part way through a long project, it is now asked again for the scenes it skipped.** Both agents wrote their whole plan in a single request, so a model that ran out of things to say part way down left the remaining scenes with no directorial intent and no shot plan at all — on a twenty-four scene piece, the Director covered eighteen. The shortfall was detected and reported honestly, and then nothing acted on it, so those six scenes were written without any of the direction the rest of the film had. There is now a follow-up request asking only for the missing scene numbers, with the entries already written sent along so the new ones do not repeat them. It is deliberately bounded: at most two extra requests, and it stops the moment a round produces nothing, because a model with nothing to say for a scene will say nothing however many times it is asked. |
