...
| --- | --- | --- |
| **2.14** | 2026-08-22 | **Regenerating one scene no longer loads the image model twice and the face-swap model twice.** A scene with two keyframes and a character opted into face swap corrected each frame the moment it was rendered, so the run went image model → swap model → image model → swap model. WanGP holds one model at a time and a load costs upwards of a minute, so four jobs were paying for four loads. Both frames are now rendered first and corrected together, which is exactly what a multi-scene batch has always done — two loads instead of four, and roughly two minutes back on every single-scene regeneration. The end frame is still rendered against the start frame, which is what keeps wardrobe, set and lighting matched across the pair; it now sees that frame before its face was corrected, as the batch path already did, and the finished frame is corrected either way. |
| **2.13** | 2026-08-22 | **"Validation failed" now says what failed, and a long concept is no longer refused in silence.** Pasting a full treatment into the concept box on the New project screen produced a red bar reading only "Validation failed" — the server had said which field was wrong and why, and the screen was discarding that half of the answer, leaving a form of twenty controls with no indication of which one to look at. The message now names the field and the reason. The concept box also shows its length against the limit as you type and refuses to submit rather than letting you find out afterwards. The limit itself was 5,000 characters, which is shorter than a page of prose; it is now 20,000, enough for a full scene-by-scene treatment. It is not unlimited on purpose: every planning agent is handed the whole concept, and one that overruns your planning model's context makes each of them quietly fall back to a generic plan, which looks like a bad film rather than an error. |
| **2.12** | 2026-08-21 | **Settings no longer claims a model you just pinned is missing from WanGP.** Changing the video or image model saved correctly but left the screen still describing the *previous* pin, so the moment you picked a new one it was reported as unused and replaced — with an explanation, "the pinned model is not in WanGP's catalogue", that the screen had no way of knowing and that was simply wrong for an installed model you had already rendered with. Two things are fixed: the resolution is now refreshed whenever anything on the page is saved, so the notice describes what you have actually chosen; and the reason is now reported by the server rather than assumed, so a pin that really is absent says so and a pin overridden for any other reason no longer borrows that wording. |
