...
| **2.53** | 2026-09-21 | **Scenes can be moved up and down the storyboard, so a pacing problem no longer means starting the project again.** Until now the running order was fixed at generation: wanting a scene later in the piece meant regenerating the storyboard, which mints new scene ids and so discards every rendered frame, pinned seed, LoRA choice and hand-edited prompt in the project. **Move up** and **Move down** now sit at the top of each scene card. They change the order and nothing else — all of that work belongs to the scene rather than to the position, so it travels with it, and no frame is ever re-rendered as a side effect. What a move genuinely invalidates is listed before it happens: scenes that open on a frame carried over from a neighbour that is no longer adjacent, and — the one nobody predicts — scenes that have crossed a costume change and whose characters are therefore now dressed differently. The confirmation offers to rewrite the affected opening prompts, off by default because those prompts may have been written by hand. Rearranging is refused while a batch is running, since the queue holds work already planned against the old order. Inserting a scene is the next step and its controls are visible but inactive. |
| **2.52** | 2026-09-20 | **A prompt you have edited by hand no longer reverts to its earlier wording after reporting that it saved.** A scene's prompt panel sent all five of its boxes on every save, and tracked unsaved work for the panel as a whole rather than per box. One box being edited therefore stopped the other four from following what was stored: the panel went on believing it was up to date while showing text the storyboard had already replaced, and wrote that stale text back the next time anything in that scene was saved — showing "Saved." every time. Prompts corrected by hand, or rewritten by **Regenerate these prompts**, **Save and rewrite prompts** or a storyboard regeneration, quietly came back as the wording they had replaced, with the first scene's end frame the usual casualty. Each box is now tracked on its own and a save sends only the boxes that actually changed, so an edit in one field neither freezes nor overwrites its neighbours. The scene card editor had the same fault and is fixed with it. |
| **2.51** | 2026-09-20 | **Keyframes can now be regenerated for a few chosen scenes at once, instead of only all-or-nothing or one card at a time.** Fixing three bad shots in a twenty-scene project meant either **Regenerate all** — which re-renders the seventeen that were fine and throws away every face swap, hand edit and imported frame on them — or opening each card in turn and rendering frames one at a time. A **Regenerate keyframes for selected scenes** panel now sits beside the clip one on the Storyboard screen, with **Regenerate all keyframes** next to **Regenerate all video** above it. Both re-render frames and stop there: no video model is loaded, so a change to an image prompt, a seed or an image LoRA costs two image jobs per scene rather than two images plus the longest job in the scene. The new attempts deliberately carry no clip, because claiming a video built from frames that no longer exist would be a lie — and because **Generate all media** already finishes exactly that state for the price of the clip alone, so the frames can be looked at before any clip is paid for. Under *continue from previous end frame* the scenes left holding a copy of a frame that has just been replaced are named on screen rather than silently added to the run. |
