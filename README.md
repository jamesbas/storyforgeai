...
| `H3_NATIVE_PROMPT_FORMAT` | Sends MiniMax H3 the labelled prompt structure from its own published guide — frame alignment, timeline, ambience and score as separate fields | **Unresolved after three A/B pairs**, each of which produced no spoken dialogue in the labelled arm where plain prose spoke the line. Audio direction, `<d>` tokenisation and prompt length have all been eliminated. The one candidate left is that the test subject is a robot the prompt says has no mouth |
