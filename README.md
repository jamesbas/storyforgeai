...
| **2.54** | 2026-09-21 | **A scene can be added anywhere in an existing storyboard, so needing one more beat no longer costs the whole project.** Changing the scene count used to mean regenerating the storyboard, which mints fresh scene ids and therefore discards every rendered frame, pinned seed, LoRA choice and hand-edited prompt in it. **Insert before** and **Insert after** now sit beside the move controls on each card, and reach every position — before the first scene, between any two, after the last. The two labels name the same gap, so it does not matter which side you approach it from. A title, a visual and an action are required and nothing else is; the piece gets one segment longer rather than the other scenes getting shorter, and appending moves the closing trim onto the new scene instead of leaving it cutting the old last scene in half. Prompts are written immediately by the deterministic builders, so the card works straight away at no cost, with the planning model offered as a tick-box rather than a toll. The new scene has no media, so the next **Generate all media** renders it alone. Where the following scene was rendered opening on a frame carried over from what is no longer its neighbour, the form says so, offers to rewrite its opening, and leaves the frame itself strictly alone. |
