...
| --- | --- | --- |
| **1.82** | 2026-08-16 | **A scene where someone gets undressed is no longer treated as a sex act.** The check that asks an explicit frame to name anatomy, contact and a position was triggered by the word "nude" appearing anywhere on the scene card — so a close-up of a character being undressed was rejected for naming no genitals, which is a demand that scene cannot meet. The prompt was correct; the rejection then patched it by appending the card's action, putting "the two men grab her top and shorts, pulling the fabric down past her hips to her ankles" — a three-step process — into a single frozen frame. Those three checks now apply only where the card actually reaches an act. The separate check on clothing the model invented still applies wherever a card leaves someone bare, so nothing is given back a costume it was not meant to have. |
