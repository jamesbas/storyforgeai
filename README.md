...
| --- | --- | --- |
| **2.39** | 2026-09-02 | **Twenty-second MiniMax H3 scenes now remain twenty seconds on every FL2VA and Ref2VA checkpoint, including PDD variants.** Some Wan2GP checkpoints save a 362-frame sliding window even when StoryForgeAI requests 481 frames, causing an apparently successful 20-second scene to stop after exactly 15.08 seconds. StoryForgeAI now sends the 481-frame window explicitly for every H3 variant instead of inheriting that mutable model setting, so PDD and non-PDD renders follow the project scene length consistently. |
| **2.38** | 2026-09-02 | **StoryForgeAI now includes a presentation package for business and technical audiences, and current Wan2GP servers no longer look misconfigured when they omit an optional discovery tool.** The repository now ships an editable eight-slide PowerPoint overview plus separate business-leader and solution-engineer infographics in both PowerPoint and 1920×1080 PNG form. They explain the visible agent crew, the plans it contributes to the storyboard, per-scene start/end keyframe and video prompts, the Wan2GP MCP execution path, and the human approval gate before assembly. A reusable Windows generator rebuilds and validates all five assets from the current product screenshots. Separately, the filesystem-read capability check now treats an advertised `wangp_list_files` tool as positive proof but treats its absence as unknown rather than unsupported. Newer Wan2GP builds can allow path-backed generation without exposing that tool, so Settings no longer raises a false warning while real filesystem refusals still return the actionable restart flag. |
| **2.37** | 2026-08-29 | **MiniMax H3 clips are no longer held to 14–15 seconds, and their prompts now match Wan2GP's published format.** Both H3 variants used to window at 362 frames, with reference mode assumed shorter still, so a project pinned to either was seeded at 15 or 14 seconds and the settings screen warned that anything longer would be silently stitched together from overlapping passes. Current Wan2GP builds window at 481 frames — 20 seconds at H3's native frame rate — so the longest clip this app offers now renders in a single pass, and both the seeded default and the on-screen advice say so instead. Reference mode also no longer claims a length cap it does not have. Separately, three details of the reference-mode prompt had drifted from the published format: the retention lines used an em dash where the guide writes a plain hyphen, the shot description opened straight onto the first shot marker instead of first saying what the whole video is, and the two audio fields sat on their own lines rather than beside their labels. And in every H3 mode, a character who spoke twice was numbered as two different speakers — the guide asks for one stable id per speaker, and a fresh id per line reads as a room full of people and can be voiced as one. A named character now keeps the same id across every line they say. Finally, a guard against a failure that would look like a bug in Wan2GP rather than here: Wan2GP starts a new video at every blank line in a prompt, so a single stray one inside a scene would quietly ask for a second clip. Both H3 formats are now stripped of blank lines before they are sent, whatever the agents or the appended clauses produced. Ordinary line breaks are untouched, because the labelled fields are supposed to sit on their own lines. |
| **2.36** | 2026-08-29 | **The Settings page is now a short list you open one part of at a time.** Three custom system prompt boxes, the character library form and the default model and LoRA pickers had turned it into a page you scrolled past rather than read, and the thing you came to change was rarely the thing at the top. Every section — Character library, LM Studio system prompts, Default models & LoRAs, and Per-project generation models — now expands and collapses, and all of them start closed, so the page opens as four headings with a line each explaining what is inside. Nothing is hidden from keyboard or screen-reader users: the headings stay in the page outline whether a section is open or shut, and expanding works from the keyboard without any custom handling. |
| **2.35** | 2026-08-29 | **Custom system prompts are now split into three scopes, because one setting was reaching agents that want opposite instructions.** 2.34 grouped story planning and render-prompt writing together, so a rule written for keyframes — count the people, name the camera, one body per person — also reached the Intake, Story Architect, Visual Bible and Storyboard Artist agents, which are explicitly told *not* to write image or video prompts yet. There is now a third box: **Agentic Canvas** for the five planning agents, **Storyboard** for the brief, arc, continuity guide and scene cards, and **Render prompts** for the two agents that actually write the text sent to the image and video models. All three are required together, as before. Because instructions in the wrong place degrade output silently rather than failing, the settings panel now warns about the four mistakes that cause it and links to a new Help section covering what StoryForgeAI already sends, why restating a character's appearance renders them twice, and why a system prompt saved inside LM Studio never reaches these calls at all. |
