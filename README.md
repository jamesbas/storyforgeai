...
| **1.88** | 2026-08-16 | **A character's description must now share the sentence that places them, and the instruction shows what that looks like.** 1.87 asked prompt agents to describe each character inline instead of relying on an appended sheet. The first storyboard written under it complied only halfway: it placed the sleeping woman in one sentence and then described her in a second, "Mara is a beautiful 52-year-old Caucasian woman with honey-blonde…". That is still two mentions of one person, and an image model that has never heard of her reads two mentions as two people — which is the whole defect. The instruction now carries a worked example of the right shape and the wrong one side by side, and states plainly that each character is mentioned exactly once. |
