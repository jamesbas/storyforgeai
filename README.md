...
| **2.57** | 2026-09-22 | **A story longer than about eighteen scenes no longer comes back half-written.** The Story Architect asked for the whole arc in one request, and a local model asked for twenty-seven beats at once simply stopped part way: sixteen beats, sixteen emotional values, and a badge reading "the model returned too few items". The beats it left out were then asked for all in one go, which failed the same way — the tail came back as nine scenes of aftermath with the last two word for word identical. Long pieces are now written a window at a time. The first request asks for the opening eight beats and says plainly that the rest will be requested, so the model stops resolving the whole story into whatever it was given, and each continuation is told where in the film it sits and how many segments still follow it. The emotional progression is now written with the beats it belongs to rather than padded from the built-in template, which is what previously sent the second half of a long film to the storyboard directed as "rising tension" over beats where the story had already ended. The finished arc is also checked for beats that repeat the beat before them, and the canvas names them — each one is a whole segment of runtime spent rendering a shot the audience has just watched. |
| **2.56** | 2026-09-22 | **An assembled cut is now named for its project and the moment it was made, instead of every project producing `rough-cut.mp4`.** A folder of finished pieces was a row of identical names, and because the download header is built from the file's own basename, that is what a browser saved too — so the second one you downloaded became `rough-cut (1).mp4` and the third was anyone's guess. Cuts are now written as `Swing Deep-2026-09-22-1005-rough-cut.mp4`, with the scored version alongside as `-final-cut.mp4`. The kind stays in the name because both cuts share a folder. Characters a filesystem reserves are stripped from the title, as they already were for the storyboard export. Re-assembling replaces the previous cut rather than accumulating them, and a project still holding an old `rough-cut.mp4` is tidied up the first time it is re-assembled. |
