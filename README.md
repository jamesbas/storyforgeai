...
| **2.48** | 2026-09-18 | **Wan2GP model lists now recover after the MCP server or network connection restarts, instead of remaining empty until StoryForgeAI is restarted.** A failed first v2 tool-discovery request was cached permanently, while the status check could still say the server was healthy without making a real request. StoryForgeAI now reconnects and retries safe discovery calls, clears failed contract probes, and verifies health against the live tool list. Both model settings screens also show the actual connection error rather than turning it into “WanGP reported no models,” and the global defaults screen now explains v2's unknown installation state consistently. |
