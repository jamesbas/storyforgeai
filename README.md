...
| --- | --- | --- |
| **2.64** | 2026-09-24 | **A character's second, third and fourth photographs are no longer dead weight, and models are no longer offered fewer reference images than they accept.** A character can store four photographs, but every path took the first one and read the rest only as a has-a-photo test — so the other three did nothing at all. Reference mode now has a **Photographs sent per character** choice: more angles of the same face give the model more to hold on to, and a reference-to-video model is the one place that genuinely helps, because it conditions identity across the whole clip rather than placing a picture in a single frame. It is off by default and each extra photograph costs roughly 5 more minutes per clip, so nobody pays for it without asking. The extra pictures are explicitly declared as the same person from different angles — without that, a second photograph of somebody reads as a second somebody. Separately, the number of reference images a model was offered had been a guess based on how its name started, which held models that advertise support for several down to two while offering two to models that accept none. It now reads what each model actually publishes, including the models that state their own limit. The keyframes still send one photograph per character: an image editor treats each reference as another thing to place in the frame, and four photographs of one woman once rendered her twice in the same shot. |
| **2.63** | 2026-09-23 | **The architecture document now describes the app that exists.** It is the single reference for how StoryForgeAI is put together, and parts of it had fallen behind the code — in one case contradicting itself. Three sections still said project state lived only in memory and was lost on restart, while the section on storage correctly described the JSON files on disk that have been the real behaviour for some time; anyone reading the wrong one would have concluded their work was not being saved, or that a container needed no volume for it. The agent contract table still described counts being demanded of a model in one shot, which several agents stopped doing when they began writing long projects a window at a time. The API map was missing around twenty routes, including the whole character library and the queue endpoints. The acceptance gate that checks every keyframe prompt before a render is spent on it — twelve separate checks, a retry that names the fault, and a deterministic repair — was not documented at all, and now has a section of its own. Assembly output names, the record's own field list, and the telemetry taxonomy were brought back in line with the code as well. |
