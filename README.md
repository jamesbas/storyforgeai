...
| **2.20** | 2026-08-23 | **The command-line tools now read your local settings, instead of quietly reporting the defaults.** Everything under `npm run` — the WanGP probes, the render preview, the planning-model checks — runs outside the app, and only the app itself loads `.env.local`. So every one of those tools was working from built-in defaults: the WanGP probes looked for a server on this machine and reported it unreachable unless you set the address by hand each time, and the context checker printed a token budget that was not the one your app uses — which is exactly how it came to state a limit of 12,000 while the app was configured for 16,000. They all read the file now. |
