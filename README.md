...
| **2.19** | 2026-08-23 | **You can now rewrite just the keyframe prompts.** The storyboard toolbar has had a *Rewrite all video prompts* button since the case that prompted it — changing the video model leaves every clip prompt phrased for the old one, while the still-frame prompts are untouched. The mirror case is just as common and had no button: change the image model, or fix a rule about how keyframes are described, and it is the clip prompts that are fine. The only choices were to rewrite both passes, paying a model call per scene to replace text that was never at fault and losing any clip wording typed by hand, or to visit twenty-four cards one at a time. There is now a *Rewrite all image prompts* button beside the other one. |
